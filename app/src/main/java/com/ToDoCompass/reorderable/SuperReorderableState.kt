...
abstract class SuperReorderableState<T>(
    ...
    internal open fun onDragStart(offsetX: Int, offsetY: Int): Boolean {
        ...
        return visibleItemsInfo
            ...
            ?.also {
                ...
                //Log.v("onDragStart", "draggingItemIndex = $draggingItemIndex")
            } != null
    }
    ...
    internal fun onDrag(offsetX: Int, offsetY: Int) {
        ...
        //if (startOffset < -100 || startOffset > viewportEndOffset+100) cancelAutoScroll()
        chooseDropItem(
            draggingItem,
    ...
    private fun calcAutoScrollOffset(time: Long, maxScroll: Float): Float {
        ...
        return when {
            delta > 0 ->
                if (endOffset - viewportEndOffset < 200f)
                    (endOffset - viewportEndOffset).coerceAtLeast(0f)
                else 0f
            delta < 0 ->
                if (startOffset - viewportStartOffset > -200f)
    ...
    internal fun itemMovedGroup(offsetX: Int, offsetY: Int, itemKey : Int, index : Int) {
        //Log.v("itemmovedGroup invoked", "index = $index")
        val targetItem = findItemAt(offsetX, offsetY)
        scope.launch {
            onMove.invoke(
                ItemPosition(index, itemKey),
                ItemPosition(targetItem?.itemIndex ?:0, targetItem?.itemKey ?:0)
            )
            scrollToItem(firstVisibleItemIndex, firstVisibleItemScrollOffset)
        }
    ...
    private fun findItemAt(offsetX: Int, offsetY: Int) : T? {
        val x: Int = offsetX
        val y: Int = offsetY
