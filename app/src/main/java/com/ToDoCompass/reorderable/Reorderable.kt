...
//SIIRRÄ NÄMÄ SUPERIIN
/*

fun Modifier.reorderable(
    channel : InteractionsChannel,
    state: ReorderableState<*>,
    columnSizeObtain : () -> IntSize
) = then(
    Modifier
        ...
        forEachGesture {
            ...
            val down = state.pointerChannel
                .receive()


            down = down.copy(
                currentPosition = offsetToGroupOffset(
            ...
            Log.v("reorderable pointerinput", "down = $down")
            //val downOffset = down?.position ?: Offset.Zero
            ...
            //Log.v("reorderable pointerinput", "downOffset = $downOffset")

            if (down != null && (
                        state.onDragStart(downOffset.x.toInt(), downOffset.y.toInt())
                        || findGroup(down.position, columnSizeObtain()) == state.group)
            ) {
                ...
                detectDrag(
                    ...
                    onDrag = { change, dragAmount ->
                        ...
                        Log.v("onDrag, group = ${state.group}", "dragAmount = $dragAmount")
                    })
            }
        }
    })
...
fun Modifier.simpleReorderable(
    state: ReorderableState<*>,
) = then(
