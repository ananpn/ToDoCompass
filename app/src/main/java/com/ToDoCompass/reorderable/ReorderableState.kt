...
abstract class ReorderableState<T>(
    ...
    group : Int,
    //val positionOfDraggingItem : (Float, Float) -> Unit
) {
    val group = group
    ...
    internal val pointerChannel = Channel<PointerInputChange>()
    ...
    var draggingDelta by mutableStateOf(Offset.Zero)
    ...
    //val _scrollOffset = MutableStateFlow(0f)
    ...
    //doesnt start drag only returns if it's possible
    internal open fun canStartDrag(offsetX: Int, offsetY: Int): Boolean {
        val x: Int
        val y: Int
        if (isVerticalScroll) {
            x = offsetX
        ...
        //val places = visibleItemsInfo.map{item -> listOf(item.left, item.right, item.top, item.bottom)}
        //Log.v("canstartdrag visibleitemsinfo", "${visibleItemsInfo.toList()}")
        //Log.v("canstartdrag visibleitemsinfo", "$places")
        //Log.v("canstartdrag visibleitemsinfo", "x = $x, y = $y")
        return visibleItemsInfo
            .firstOrNull { x in it.left..it.right && y in it.top..it.bottom } != null
    }
        ...
        chooseDropItem(
            ...
            if (targetItem.itemIndex == firstVisibleItemIndex || draggingItem.itemIndex == firstVisibleItemIndex) {
                scope.launch {
                    ...
                    //Log.v("ondrag scroll","choosedropitem firstVisibleItemIndex= $firstVisibleItemIndex, firstVisibleItemScrollOffset = $firstVisibleItemScrollOffset")
                    if (group in 0..3) {
                        scrollToItem(firstVisibleItemIndex, firstVisibleItemScrollOffset)
                    }
/*
                    if (group == 69 && draggingDelta.y > 0) {
                        scrollToItem(firstVisibleItemIndex, draggingItem.top)
                    }
                    */
                }
            } else {
            ...
            if (!isMovingGroup){
                scope.launch{
                    dragCancelledAnimation.dragCancelled(
                        position = ItemPosition(draggingItem.itemIndex, targetItem.itemKey),
                        offset = Offset(x =0f, y= (targetItem.top-draggingItem.top).toFloat())
                    )
                }
            }
        ...
        with(calcAutoScrollOffset(0, maxScrollPerFrame)) {
            //Log.v("reordst onDrag", "with(calcAutoScrollOffset")
    ...
    private fun autoscroll(scrollOffset: Float) {
        //Log.v("reordst autoscroll", "launch, scrolloffset = $scrollOffset")
        if (scrollOffset != 0f) {
            ...
            autoscroller = scope.launch {
                ...
                while (scroll != 0f && autoscroller?.isActive == true) {
                    ...
                    //_scrollOffset.value = scroll
                    //Log.v("reordst autoscroll", "scroll = $scroll")
    ...
    protected open fun findTargets(x: Int, y: Int, selected: T): List<T> {
        ...
        //Log.v("reordstate", "distances = $distances")
        return targets
    }
    ...
    protected open fun chooseDropItem(draggedItemInfo: T?, items: List<T>, curX: Int, curY: Int): T? {
        ...
        items.fastForEach { item ->
            ...
            if (isVerticalScroll && dy > 0) {
                //Log.v("reordstate choosedropitem",
                //     "dy >0, dy = $dy firstVisibleItemIndex = ${firstVisibleItemIndex}")
        ...
        with(target) {
            if (this != null){
                originalPositionOfMoved = this.top
                keyOfMoved = this.itemKey as Int
            }
        }
        */
/*
        Log.v("reordstate choosedropitem",
            "draggediteminfo.top = ${draggedItemInfo.top}")
        Log.v("reordstate choosedropitem",
            "target.top - draggediteminfo.top ${target?.top ?:0 - draggedItemInfo.top}")
        */
        if (group == 69 && target?.itemIndex == firstVisibleItemIndex+1 && dy > 0) scope.launch{
            if (firstVisibleItemIndex <= 1)
                scrollToItem((firstVisibleItemIndex).coerceAtLeast(0), -draggedItemInfo.bottom)
            else
                scrollToItem((firstVisibleItemIndex).coerceAtLeast(0), -draggedItemInfo.top)
        }
        return target
    }

    //var originalPositionOfMoved by mutableStateOf(0)
    //var keyOfMoved by mutableStateOf(0)
    ...
    private fun calcAutoScrollOffset(time: Long, maxScroll: Float): Float {
        ...
        //Modified to disable scroll when outside the column
        val offsetLimit = abs(viewportEndOffset-viewportStartOffset).toFloat()/3.3f
        return when {
            delta > 0 ->
                if (endOffset - viewportEndOffset < offsetLimit)
                    (endOffset - viewportEndOffset).coerceAtLeast(0f)
                else 0f
            delta < 0 ->
                if (startOffset - viewportStartOffset > -offsetLimit)
                    (startOffset - (viewportStartOffset+offsetLimit/5f)).coerceAtMost(0f)
                else 0f
            else -> 0f
        }
    ...
    companion object {
        ...
        private fun interpolateOutOfBoundsScroll(
            ...
            //Log.v("Reordstata", "interpoloutofoboundsscroll launch")
            //Log.v("Reordstata interpol", "viewSize = $viewSize, viewSizeOutOfBounds = $viewSizeOutOfBounds, time = $time, maxScroll = $maxScroll")
    ...
    private var isMovingGroup : Boolean = false
    private var isTargetIndexIncremented : Boolean = false
    //private var groupMoveTargetItem : T? = null

    fun letIsMovingGroup(new : Boolean){
        isMovingGroup = new
    }

    internal fun itemMovedGroup(pointerPosition : Offset,
                                realBoxPosition : Offset,
                                fakeBoxPositionOffset : Offset,
                                newItemKey : Int,
    ) {

        scope.launch {
            visibleItemsInfo
                .firstOrNull { it.itemKey == newItemKey }
                ?.also {
                    selected = it
                    draggingItemIndex = it.itemIndex

                }
            val initialBoxPosition = -realBoxPosition + pointerPosition
            draggingDelta = initialBoxPosition+fakeBoxPositionOffset

        }
    }

    private fun findItemAt(offset : Offset) : T? {
        //visibleItemsChanged()
        //val x: Int = offset.x.toInt()
        val y: Int = offset.y.toInt()
        val output = visibleItemsInfo.firstOrNull() {
            y in it.top..it.bottom }
            ?:visibleItemsInfo.firstOrNull()
        return output
    }

    fun findItemIndexAt(offset : Offset, isDragging : Boolean) : Int? {
        isTargetIndexIncremented = false
        //val x: Int = offset.x.toInt()
        val y: Int = offset.y.toInt()
        var output : Int?
        visibleItemsInfo.firstOrNull() {
            y in it.top..it.bottom
                ...
                output = it?.itemIndex
                //groupMoveTargetItem = it
        }
            ?:visibleItemsInfo.firstOrNull()
        if (output != null
            && output==firstVisibleItemIndex
            && firstVisibleItemScrollOffset>0 && isDragging)
        {
            output = output!! + 1
            isTargetIndexIncremented = true
        }
        return output
    }

    fun findItemAtIndex(index : Int) : ItemPosition {
        val key =visibleItemsInfo.firstOrNull() {
            it.itemIndex == index }?.itemKey
        return ItemPosition(index, key)
    }

    fun scrollToNewItem(newItemIndex : Int) {
        val targetScrollIndex = when {
            newItemIndex == 0 -> 0
            (firstVisibleItemScrollOffset>0 && newItemIndex == firstVisibleItemIndex) -> newItemIndex
            (firstVisibleItemScrollOffset>0
                    && newItemIndex == firstVisibleItemIndex+1
                    && isTargetIndexIncremented) -> newItemIndex
            else -> firstVisibleItemIndex
        }

        val targetScrollOffset = when {
            newItemIndex == 0 -> 0
            (newItemIndex == firstVisibleItemIndex) -> 0
            (newItemIndex == firstVisibleItemIndex+1 && isTargetIndexIncremented) -> 0
            else -> firstVisibleItemScrollOffset
        }


        //Log.v("itemMovedGroup", "offsetX = $offsetX, offsetY = $offsetY")
        //Log.v("itemMovedGroup", "draggedItemOffset.x = ${draggedItemOffset.x}, draggedItemOffset.y = ${draggedItemOffset.y}")
        //Log.v("itemMovedGroup", "initialBoxPosition() = ${initialBoxPosition()}")



        //else scrollToItem(0,0)
        scope.launch{
            scrollToItem(targetScrollIndex, targetScrollOffset)
        }
    }

    
    fun ItemsOverFlowing(count : Int) : Boolean{
        if (visibleItemsInfo.lastIndex < count){
            return true
        }
        else return false
